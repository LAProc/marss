...
int ReorderBufferEntry::issue() {
    ...
    //MOCH
    bool pref = isprefetch(uop.opcode);
    ...
    if unlikely ((ra.flags | rb.flags | rc.flags) & FLAG_INV) {
        ...
        if unlikely (ld|st) {
            ...
            //MOCH
            //It could be the case that either lsq or origvirt is not valid
            issueprefetch(state, origvirt, radata, rbdata, rcdata, uop.cachelevel);
            /*if(lsq==NULL)   std::cout<< "lsq null !" << std::endl;  
            std::cout<< "origvirt is " << origvirt<< std::endl;
            //completed = issueload(*lsq, origvirt, radata, rbdata, rcdata, pteupdate);*/
